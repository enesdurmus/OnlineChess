...
public class Board extends javax.swing.JFrame {
    ...
    private int checkMateCounter = -1;
    private Timer timer;
    ...
    private Piece FindPieceWithOutSelect(String name) {
        myPieces.stream().filter((piece) -> (name.equals(piece.getName()))).forEachOrdered((piece) -> {
            tempPiece = piece;
        });
        return tempPiece;
    ...
    public void ReadCheckMate(Message msg) {
        timer.StopTimer();
        resultLabel.setText((String) msg.content);
        jPanel1.setVisible(true);
    }

    private void MovePiece(int square) {
        if (selectedPiece.getName().equalsIgnoreCase(Side.charAt(0) + "King")) {
            CheckControl(square);
        }
        if (!check) {
            if (!selfCheck) {
            ...
            King myKing = (King) FindPieceWithOutSelect(Side.charAt(0) + "King");
            if (!check) {

                if (selectedPiece.getName().equals(myKing.getName())) {
                    if (selectedPiece.Move(square)) {
                        System.out.println(selectedPiece.getName() + " is moving to " + square);
                        SendMoveInfToServer(selectedPiece.getName(), square);
    ...
    //Check if the opponent checks.
    private void CheckControl() {
        check = false;
        King myKing = (King) FindPieceWithOutSelect(Side.charAt(0) + "King");
        opponentPieces.stream().map((opponentPiece) -> {
            opponentPiece.SetSquaresCanMove(allPieces);
            return opponentPiece;
        }).forEachOrdered((opponentPiece) -> {
            for (Piece attackablePiece : opponentPiece.getAttackablePieces()) {
                if (attackablePiece.getSquare() == myKing.getSquare()) {
                    System.out.println("check from opponent");
                    check = true;
                }
            }
        });
    ...
    //Check king if the position it goes make itself check
    private void CheckControl(int square) {
        selfCheck = false;
        King myKing = (King) FindPieceWithOutSelect(Side.charAt(0) + "King");
        opponentPieces.stream().map((opponentPiece) -> {
            opponentPiece.SetSquaresCanMove(allPieces);
            ...
            for (int squareCanMove : opponentPiece.getSquaresCanMove()) {
                if (squareCanMove == square) {
                    System.out.println("check myself");
                    selfCheck = true;
                }
            }
        });
    ...
    private void CheckMateControl() {
        King myKing = (King) FindPieceWithOutSelect(Side.charAt(0) + "King");
        myKing.SetSquaresCanMove(allPieces);
        checkMateCounter = myKing.getSquaresCanMove().size();
        opponentPieces.stream().map((opponentPiece) -> {
            opponentPiece.SetSquaresCanMove(allPieces);
            return opponentPiece;
        }).forEachOrdered((opponentPiece) -> {
            for (Integer integer : myKing.getSquaresCanMove()) {
                for (int squareCanMove : opponentPiece.getSquaresCanMove()) {
                    if (squareCanMove == integer) {
                        checkMateCounter--;
                    }
                }
            }
        ...
        if (checkMateCounter == 0) {
            ...
            timer.StopTimer();
            checkMate = true;
            resultLabel.setText("Kaybettiniz...");
            Message msg = new Message(Message.Message_Type.CheckMate);
            msg.content = "Kazandınız";
            Client.Send(msg);
        }
    }

    public void CheckMateWTime() {
        jPanel1.setVisible(true);
        timer.StopTimer();
        checkMate = true;
        resultLabel.setText("Kaybettiniz...");
    ...
    private void initComponents() {
        ...
        resultLabel.setBackground(new java.awt.Color(162, 178, 159));
        resultLabel.setFont(new java.awt.Font("Times New Roman", 2, 24)); // NOI18N
        resultLabel.setForeground(new java.awt.Color(121, 135, 119));
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                    .addContainerGap()
                    .addComponent(resultLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 237, Short.MAX_VALUE)
                    .addContainerGap()))
        );
